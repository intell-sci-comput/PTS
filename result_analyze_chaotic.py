...
def prun_constant(expr_num_sympy, n_digits=6):
    ...
    def process_term(term):
        if isinstance(term, sp.Float) or isinstance(term, sp.Rational):
            float_value = float(term)
            if abs(float_value) < epsilon:
                return sp.sympify("0")
            else:
                return sp.Float(round(float_value, n_digits))
        elif isinstance(term, sp.Mul):
            coeff, rest = term.as_coeff_Mul()
            if isinstance(coeff, (sp.Float, sp.Rational)):
                float_value = float(coeff)
                if abs(float_value) < epsilon:
                    return sp.sympify("0")
                else:
                    rounded_value = sp.Float(round(float_value, n_digits))
                    return rounded_value * rest
        return term

    return expr_num_sympy.replace(
        lambda x: isinstance(x, (sp.Float, sp.Rational, sp.Mul)), process_term
    )
